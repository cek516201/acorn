...
//		PreparedStatement ps = null;
		...
		try (PreparedStatement ps = conn.prepareStatement("update member set num=?, name=?, addr=? where num=1")) {
//			// 실행할 sql문
//			String sql = "update member" + " set num=?, name=?, addr=?" + " where num=1";
...
//			ps = conn.prepareStatement(sql);
			...
			// sql문 실행하기
			System.out.println(ps.executeUpdate() + "개의 회원 정보를 수정했습니다");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
