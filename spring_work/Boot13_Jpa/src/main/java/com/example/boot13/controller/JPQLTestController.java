...
public class JPQLTestController {
	...
	public List<String> oracle(String query) {
	...
	public List<Object[]> array(String query) {
		/*
		 * select 된 row 하나의 정보를 Object[] 에 담고 row 가 여러개이니까 Object[] 객체가 여러개가 필요하고 해당 객체를
		 * 모두 List 에 담으면 List<Object[]> type 이 된다.
		 */
	...
	public List<DeptDto> dept(String query) {
		// 미리작성된 query 문을 이용해서 Dept entity 로 부터 결과를 얻어와서
		List<DeptDto> list = null;
		...
		try {
			/*
			 * .createQuery() 메소드의 2번째 인자로 전달한 Entity 클래스가 리턴되는 TypedQuery 객체의 Generic type
			 * 이 된다.
			 */
