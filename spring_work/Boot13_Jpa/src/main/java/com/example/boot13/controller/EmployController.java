...
//		List<EmpDeptDto> list = empRepo.findByEnameContaining(keyword).stream().map(EmpDeptDto::toDto).toList();

//		List<EmpDeptDto> list = empRepo.findByEnameContainingOrJobContaining(keyword, keyword).stream()
//				.map(EmpDeptDto::toDto).toList();

		// 부서번호에 대해서 오름차순, sal에 대해서 내림차순
		Sort sort = Sort.by(Order.asc("dept.deptno"), Order.desc("sal"));

		// 1페이지 내용, 한 페이지에 3개, 정렬
		Pageable pageable = PageRequest.of(1 - 1, 3, sort);

		List<EmpDeptDto> list = empRepo.findByEnameContaining(keyword, pageable).stream().map(EmpDeptDto::toDto)
				.toList();
	...
	public String list2(Model m, int pageNum) {
		 ...
		 */
		PageRequest.of(pageNum - 1, PAGE_ROW_COUNT, sort);
	...
	public String list(Model m) {
		...
		// List<EmpDeptDto>
		// list=empRepo.findAll().stream().map(EmpDeptDto::toDto).toList();
